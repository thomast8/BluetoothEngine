...
A small, focused Swift package for reading **Bluetooth LE pulse oximeters** (SpO₂ + pulse rate) on
macOS and iOS, with a CLI sniffer and a SwiftUI debug app. A reusable standalone biofeedback input
for breath-hold / apnea training.
...
- **`BluetoothEngine`** (library) — a `@MainActor` `BLECentral` over CoreBluetooth with `AsyncStream`
  ...
  `SupportedDevices` declares what the engine can talk to (so an app can scan filtered to compatible
  devices instead of listing every AirPod in the room). The engine declares capability; the app owns
  presentation/policy.
